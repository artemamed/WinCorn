...
export default function Setting() {
  ...
  const splitNameAndUpdateInput = React.useCallback(() => {
  ...
  const SubmitChanges = useMutation({
    mutationFn: ({ updatedFields }: { updatedFields: Record<string, unknown> }) => {
  ...
  const handleSubmitLogin = async (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        const errString: string = (error as { errors?: string[] }).errors?.[0] || 'Validation error';
      ...
      <div className="px-5 space-y-4">
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <button
            ...
            disabled={isloading}
            className="w-full text-white bg-green-500 hover:bg-green-600/75 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center "
          >
            Update Profile
          </button>
        </div>
