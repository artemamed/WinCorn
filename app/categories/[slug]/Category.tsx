...
const Category = ({ slug }: { slug: string }) => {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 rounded-xl shadow-lg bg-gray-300 transition-all duration-300">
      <div className="flex flex-col lg:flex-row gap-6">
        {/* Desktop Categories */}
        <div className="hidden lg:block lg:w-1/3 xl:w-1/6">
        ...
        <div className="flex flex-col md:items-start lg:w-2/3 xl:w-3/4 pl-[5rem]">
            ...
            {allSubCategories &&
              allSubCategories.subcategories.map((sub: { sub_category: { sub_category_slug: string; sub_category_name: string } }, index: number) => (
                <Link
                  ...
                  key={index}
                  className="flex flex-col justify-between h-36 border border-gray-200 rounded-lg shadow-md transition-transform duration-200 transform hover:scale-105 bg-white hover:shadow-lg cursor-pointer"
                >
                  <div className="flex-1 flex items-center justify-center">
                    <div className="text-gray-800 font-semibold text-center hover:text-green-500">{sub.sub_category.sub_category_name}</div>
                  </div>
